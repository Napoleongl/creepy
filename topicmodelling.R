...
topic_data    <- expand_grid(ntopics = seq.int(from = 4, to = 20, by = 2),
                             types = list(c("NOUN"),c("ADJ", "NOUN"),c("VERB", "NOUN"),
                                          c("VERB", "ADJ", "NOUN")))

bigram_data   <- expand_grid(ntopics = seq.int(from = 4, to = 20, by = 2), 
                             types = list(c("NOUN", "Bigrams")))

clean_models  <- mclapply(X = 1:nrow(topic_data),
                          FUN = function(x){
                            LDA_modeller(data = ud_clean, 
                                         types = unlist(topic_data$types[x]), 
                                         ntopics = topic_data$ntopics[x])
                          },
                          mc.cores = 3L)

bigram_models <- mclapply(X = 1:nrow(bigram_data),
                          FUN = function(x){
                            LDA_ngram_modeller(data = ud_clean, 
...
save(clean_models, bigram_models, topic_data, bigram_data, file = "LDA_models.RData")
